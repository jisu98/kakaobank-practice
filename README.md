...
이 프로젝트는 AI 페어 프로그래밍으로 진행하되, AI가 일관된 구조로만 코드를 생성하도록
사전에 제약과 규칙을 설계하는 데 중점을 두었습니다.

- 아키텍처 경계와 레이어 의존성 규칙(domain의 프레임워크 의존 금지 등)을 먼저 정의
- 작업을 커밋 단위로 분할하고, 각 단계의 범위를 먼저 확정한 뒤 진행
- 커밋·코딩 컨벤션과 핵심 비즈니스 로직을 명세하여 구현의 일관성을 강제
